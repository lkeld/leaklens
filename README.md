...
6.  [Getting Started / Installation (Self-Hosting)](#getting-started--installation-self-hosting)
    ...
    *   [Backend Setup (`api_server/`)](#1-backend-api_server)
    *   [Frontend Setup (`webapp/`)](#2-frontend-webapp)
7.  [API Reference](#api-reference)
8.  [In-Depth Cryptography & Protocol Details](#in-depth-cryptography--protocol-details)
    *   [The Privacy Challenge](#1-the-privacy-challenge-in-password-checking)
    *   [Elliptic Curve Cryptography (ECC) Fundamentals](#2-elliptic-curve-cryptography-ecc-fundamentals)
    *   [Initial Credential Processing](#3-initial-credential-processing)
    *   [Hashing to the Elliptic Curve](#4-hashing-to-the-elliptic-curve-hashtothecurvelookup_hash)
    *   [The Commutative Cipher (`ECCommutativeCipher`)](#5-the-commutative-cipher-eccommutativecipher)
    *   [The Google API Protocol Flow (Detailed)](#6-the-google-api-protocol-flow-revisited-with-keys)
    *   [Security & Privacy Implications](#7-security--privacy-implications)
...
LeakLens employs a sophisticated cryptographic protocol to verify credentials against known leaks without compromising user privacy. The operational flow is as follows:
...
## Getting Started / Installation (Self-Hosting)
...
## In-Depth Cryptography & Protocol Details
...
*   **Encryption (`encrypt(lookup_hash)` by LeakLens Server):**
    1.  Map the input to a curve point: $P = \text{hashToTheCurve}(\text{lookup_hash})$.
