...
graph LR
    ...
    F1 --> G(Google API);
    C2 --> G;
    G --> H{4. Second Encryption (key $k_G$)};
    H --> H1["$E_{GS} = k_G \cdot E_S$ <br> (reencrypted_lookup_hash)"];
    H --> H2["Encrypted Leak Prefixes <br> ($k_G \cdot L_j$)"];
    H1 --> I(LeakLens Server);
    H2 --> I;
    I --> J{5. Decryption & Comparison};
    J --> J1["$P_G = k_S^{-1} \cdot E_{GS} = k_G \cdot P$"];
